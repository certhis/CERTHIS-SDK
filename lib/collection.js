module.exports = function (request, Web3, params) {
  ...
  const { multiplyBigIntByFloat, decodeLogsUntilEventName } =
    require("./extra")(Web3);
  var format_collection = function (
    obj,
    contract_id = null,
    v2 = null,
    v3 = null
    ...
    if (v2 != null || v3 != null) {
      ...
      if (v3 != null) {
        return_obj.custom_allowance =
          obj.custom_allowance == undefined ? false : obj.custom_allowance;
      }
    }
  ...
  var one = async function (
    ...
    if (typeof collection_index == "object" && collection_index != null && collection_index != undefined && collection_index != "") {
          ...
          const signedTransaction = await web3.eth.accounts.signTransaction(
            {
              ...
              gas: multiplyBigIntByFloat(functionGasFees, mulParam),
              gasPrice: multiplyBigIntByFloat(currentGasPrice, mulParam),
              gasLimit: multiplyBigIntByFloat(functionGasFees, mulParam*1.1),
  ...
  return {
    ...
    async transferOwnership({
      ...
      return new Promise(async (resolve, reject) => {
        const Contract = require("./contract")(request);
        var collection = await one({ collection_index: collection_index });
        ...
        var transfert_owner =
          contract_collection.methods.transferOwnership(new_owner);

        const functionGasFees = await transfert_owner.estimateGas({
          from: current_address,
        });
          ...
          const signedTransaction = await web3.eth.accounts.signTransaction(
            {
              ...
              gasPrice: multiplyBigIntByFloat(currentGasPrice, mulParam),
              gasLimit: multiplyBigIntByFloat(functionGasFees, mulParam*1.1),
              data: contract_collection.methods
          ...
          await web3.eth
            ...
            .on("confirmation", async ({ confirmations, receipt }) => {
              if (confirmations >= params.confirm_need && !confirmationFlag) {
                ...
                var collection_object = await one({
                  collection_index: collection_index,
                  update: 1,
                });
