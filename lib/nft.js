...
module.exports = function (request) {
  ...
  var many = async function (params) {
    return await request.get("nfts", params);
  };
  ...
  var mint = async function (
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (collection.collection_full == 2) {
        var array_merkle = JSON.parse(collection.collection_price)[nft.nft_id];

        var leaf_split = array_merkle.split(":");
      ...
      obj_trans.gasLimit = Math.ceil(functionGasFees * 1);
      obj_trans.gas = functionGasFees;
      obj_trans.gasPrice = Math.ceil(currentGasPrice);
      var conf = 0;
      var nft_id = null;

      var nonce =  await web3.eth.getTransactionCount(current_address,"pending");
      ...
      {
        nonce_array[current_address+"_"+nonce] = nonce_array[current_address+"_"+nonce] + 1;
      }
          ...
          if (conf == 3) {
            ...
            try {
             var nft_object =  await one(
              ...
              resolve({ nft_object : nft_object, transactionHash: receipt.transactionHash});
            } catch (err) { 
