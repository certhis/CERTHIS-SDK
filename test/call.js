...
async function getOneNFT() {
  ...
  var getOneNFT = await certhis.nft.one(
    0,
...
async function getOneUser() {
  var getOneUser = await certhis.user.one(
    "0x7c78A82f71d6D7C9F14c4930F786F74Cd8171238"
  );
  return getOneUser;
}
...
async function getTransactionCollection() {
  var collection = await getOneCollection();
  var getTransactionCollection = await certhis.transaction.collection(
    collection.id
  );
...
async function getTransactionNFT() {
  var collection = await getOneCollection();
  var getTransactionNFT = await certhis.transaction.nft(
    0,
    collection.collection_address
  );
